$(function(){
	...
	$('body').on('click', '#addDist', function(){
		$('#targetDist').append('<li id="close-dist"><input class="span5" id="appendedInputButton" type="text" name="distribution[]" placeholder="distribusikan kepada"><button class="btn btn-danger remove" type="button"><i class="fam-cancel"></i></button></li>');
		$(".remove").click(function(e) {
	...
	/* $('#categories').on("change", function() {
		var cat_pro = $(this).val();
		var lock = '.'+cat_pro;
		var target = $('#close').attr('data-id');
