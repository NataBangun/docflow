...
//    |   |   |   |   |- plugin.min.js
$nik = isset($_GET['nik'])?$_GET['nik']:$_SESSION['nik'];
$base_url="http://".$_SERVER['HTTP_HOST'];  // DON'T TOUCH (base url (only domain) of site (without final /)).
$upload_dir = '/docflow2/uploads/'.$nik.'/'; // path from base_url to base of upload folder (with start and final /)
$current_path = '../../uploads/'.$nik.'/'; // relative path from filemanager folder to upload folder (with final /)
